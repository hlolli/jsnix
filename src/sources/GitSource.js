  ...
  slasp.sequence(
    [
      function (callback) {
        ...
        temp.mkdir(
          "node2nix-git-checkout-" +
            self.dependencyName.replace("/", "_slash_"),
          callback
        );
      },
      ...
      function (callback) {
        ...
        process.stderr.write(
          "Parsing the revision of commitish: " + branch + "\n"
        );
        ...
        gitRevParse.on("close", function (code) {
          if (code != 0) self.rev = ""; // If git rev-parse fails, we consider the commitIsh a branch/tag.
      ...
      function (callback) {
        if (commitIsh !== null && self.rev == "") {
          ...
          process.stderr.write(
            "Parsing the revision of commitish: origin/" + commitIsh + "\n"
          );

          /* Resolve the hash of the branch/tag */
          var gitRevParse = child_process.spawn(
            "git",
            ["rev-parse", "origin/" + commitIsh],
            {
              cwd: repositoryDir,
            }
          );
          ...
          gitRevParse.on("close", function (code) {
            if (code == 0) {
              ...
              callback(
                "Cannot find the corresponding revision of: " + commitIsh
              );
            }
          });
        } else {
      ...
      function (callback) {
        ...
        var gitSubmoduleUpdate = child_process.spawn(
          "git",
      ...
      function (callback) {
        ...
        var nixHash = child_process.spawn("nix-hash", [
          "--type",
          "sha256",
          repositoryDir,
        ]);
 ...
 */
GitSource.prototype.convertFromLockedDependency = function (
  dependencyObj,
  callback
) {
