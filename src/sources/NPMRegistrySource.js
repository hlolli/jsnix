 ...
 * @param {Boolean} useImpureNpmCache Specifies that node2nix expression generator should cache the packages fetched from npm
 */
function NPMRegistrySource(
  baseDir,
  dependencyName,
  versionSpec,
  registries,
  ...
  this.useImpureNpmCache = useImpureNpmCache;
}
...
NPMRegistrySource.prototype.fetch = function (callback) {
  ...
  if (self.versionSpec == "")
    // An empty versionSpec translates to *
    self.versionSpec = "*";
    ...
    [
      function (callback) {
        ...
        if (self.versionSpec.startsWith("npm:")) {
          var npmProtocolVersion = self.versionSpec
            .replace(/npm:@?/i, "")
            .match(/@(.*)/i);
          ...
          if (
            ...
            npmProtocolPath = npmProtocolPath.replace(
              npmProtocolVersion[0],
              ""
            );
          }
        ...
        if (self.dependencyName.startsWith("@")) {
          var scope = self.dependencyName.slice(
            0,
            self.dependencyName.indexOf("/")
          );
      ...
      function (callback, result) {
        ...
        if (semver.validRange(self.versionSpec, true) === null) {
          // If the version specifier is not a valid semver range, we consider it a tag which we need to resolve to a version
        ...
        var resolvedVersion = semver.maxSatisfying(
          versionIdentifiers,
          version,
          true
        );
        ...
        // self.config.dependencies = Object.assign(
        //   {},
        //   self.config.devDependencies || {},
        //   self.config.dependencies
        // );
        self.config.name = self.npmProtocolPath
          ? self.dependencyName
          : self.config.name;
        self.identifier = self.config.name + "-" + self.config.version;

        // prevent infinite loop when a package depends on itself
        delete (self.config.dependencies || {})[self.config.name];

        if (
          self.stripOptionalDependencies &&
          self.config.optionalDependencies !== undefined &&
          self.config.dependencies !== undefined
        ) {
          /*
 ...
 */
NPMRegistrySource.prototype.convertFromLockedDependency = function (
  dependencyObj,
  callback
) {
  this.config = {
