 ...
 */
Source.constructSource = function (
  registries,
  baseDir,
  outputDir,
  dependencyName,
  ...
  if (parsedVersionSpec !== null) {
    // If the version is valid semver range, fetch the package from the NPM registry
    return new NPMRegistrySource(
      baseDir,
      dependencyName,
      parsedVersionSpec,
      registries,
    ...
    return new GitSource(
      baseDir,
      dependencyName,
      GitSource.composeGitURL("git://github.com", parsedUrl)
    );
    ...
    return new GitSource(
      baseDir,
      dependencyName,
      GitSource.composeGitURL("https://gist.github.com", parsedUrl)
    );
    ...
    return new GitSource(
      baseDir,
      dependencyName,
      GitSource.composeGitURL("git://bitbucket.org", parsedUrl)
    );
    ...
    return new GitSource(
      baseDir,
      dependencyName,
      GitSource.composeGitURL("https://gitlab.com", parsedUrl)
    );
  } else if (
...
Source.prototype.fetch = function (callback) {
  callback(
    "fetch() is not implemented, please use a prototype that inherits from Source"
  );
};
 ...
 *     an error message.
 */
Source.prototype.convertFromLockedDependency = function (
  dependencyObj,
  callback
) {
  callback(
    "convertFromLockedDependency() is not implemented, please use a prototype that inherits from Source"
  );
};
 ...
 */
Source.prototype.toNixAST = function (args) {
  const pkgName = this.config.name.replace("@", "_at_").replace("/", "_slash_");
