...
function Package(
  ...
  this.source = Source.constructSource(
    deploymentConfig.registries,
    baseDir,
    deploymentConfig.outputDir,
    name,
    versionSpec,
    deploymentConfig.stripOptionalDependencies,
    deploymentConfig.useImpureNpmCache
  );
 ...
 */
Package.prototype.findMatchingProvidedDependencyByParent = function (
  name,
  versionSpec
) {
  if (this.parent === null) {
...
Package.prototype.bundleDependency = function (dependencyName, pkg) {
  ...
  if (this.deploymentConfig.flatten) {
 ...
 *     fails.
 */
Package.prototype.bundleDependencies = function (
  ...
  if (dependencies === undefined) {
    ...
    slasp.fromEach(
      ...
      function (dependencyName, callback) {
        ...
        var parentDependency = self.findMatchingProvidedDependencyByParent(
          dependencyName,
          versionSpec
        );
        ...
        if (self.isBundledDependency(dependencyName)) {
          ...
          var pkg = new Package(
            ...
            versionSpec,
            self.source.baseDir,
            true /* Never include development dependencies of transitive dependencies */,
            self.sourcesCache
          );
...
Package.prototype.resolveDependenciesAndSources = function (callback) {
  ...
  slasp.sequence(
    [
      function (callback) {
        self.bundleDependencies(
          resolvedDependencies,
          self.source.config.dependencies,
          callback
        );
      },
      ...
      function (callback) {
        ...
        if (self.production) {
          ...
          self.bundleDependencies(
            resolvedDependencies,
            self.source.config.devDependencies,
            callback
          );
        }
      },
      ...
      function (callback) {
        if (self.deploymentConfig.includePeerDependencies) {
          ...
          self.bundleDependencies(
            resolvedDependencies,
            self.source.config.peerDependencies,
            callback
          );
        } else {
          callback();
...
Package.prototype.resolveDependenciesFromLockedDependencies = function (
  dependencyObj,
  callback
) {
  var self = this;
    ...
    slasp.fromEach(
      ...
      function (dependencyName, callback) {
        ...
        if (dependency.bundled) {
          // Bundled dependencies should not be included
          callback();
        } else if (
          self.deploymentConfig.stripOptionalDependencies &&
          dependency.optional
        ) {
          // When the stripping optional dependencies feature has been enabled, remove all optional dependencies
          callback();
        } else if (self.production && dependency.dev) {
          // Development dependencies should not be included in production mode
          callback();
        } else {
          var pkg = new Package(
            self.deploymentConfig,
          ...
          slasp.sequence(
            [
              ...
              function (callback) {
                self.sourcesCache.addSource(pkg.source);
                pkg.resolveDependenciesFromLockedDependencies(
                  dependency,
                  callback
                );
              },
...
Package.prototype.resolveDependencies = function (callback) {
  if (this.lock === undefined) {
    // If no lock file is present, let the tool fetch all dependencies and metadata
    this.resolveDependenciesAndSources(callback);
  } else {
    // If we have a lock file, use that to generate Nix expressions
...
Package.prototype.generateDependencyAST = function () {
  ...
  Object.keys(self.providedDependencies)
    .sort()
    .forEach(function (dependencyName) {
      ...
      var ref = new nijs.NixFunInvocation({
